...
DBS.loadMods = async function () {
  ...
  require('fs')
    ...
    .forEach((mod) => {
      const fetchedMod = require(require('path').join(
        __dirname,
        `mods/${mod}`
      ));
      fetchedMod.init(DBS);
...
DBS.callNextAction = async function (command, message, args, index) {
  try {
    ...
    DBS.logError({
      level: 'error',
      message:
        'Call next action: ' + '[' + message.content + '] ' + error.stack,
    });
  }
};
...
DBS.loadVars = async function () {
  ...
  DBS.globalVars = {};
  try {
    var rawserverdata = fs.readFileSync(
      __dirname + '/BotData/variables/servervars.json'
    );
  ...
  try {
    var rawglobaldata = fs.readFileSync(
      __dirname + '/BotData/variables/globalvars.json'
    );
